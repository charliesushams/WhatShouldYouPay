...
def extract_row(listing: list) -> dict:
    try:
        ...
        return {
            "id": listing[config.IDX_LISTING_ID],
            "title": emoji.replace_emoji(listing[config.IDX_TITLE], ""),
            "bedrooms": features[-2],
            "sqft": features[-1],
...
def find_feat_index(listing: list, feat_constant: int) -> list:
    ...
    if feat_constant == 5:
        ...
        raise ListingDataError("unknown feature requested...double check constants")
